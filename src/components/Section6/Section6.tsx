            ...
            <div id="content" className="w-full h-auto flex flex-row gap-20 items-center justify-center">
                <form className="w-full lg:md:w-1/2 h-full p-10 flex flex-col gap-10" onSubmit={handleSendEmail}>
                    ...
                    <div id="content" className='h-auto flex flex-col gap-5'>
                        <div id='userName'>
                            <input className='bg-black/30 w-full h-12 p-5 outline-none rounded-md text-black font-normal text-xl placeholder:text-slate-900' placeholder='Your name' type="text" name='userName' value={userName} onChange={handleNameChange}/>
                        </div>
                        <div id='emailAddress'>
                            <input className='bg-black/30 w-full h-12 p-5 outline-none rounded-md text-black font-normal text-xl placeholder:text-slate-900' placeholder='Your email address' type="email" name='emailAddress' value={emailAddress} onChange={handleEmailChange}/>
                        </div>
                        <div id='message'>
                            <textarea className='bg-black/30 w-full h-44 p-5 outline-none rounded-md text-black font-normal text-xl placeholder:text-slate-900' placeholder='Your message' name='message'value={message} onChange={handleMessageChange}/>
                        </div>
